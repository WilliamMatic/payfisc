...
interface StatusAgentModalProps {
  agent: AgentType;
  processing: boolean;
  onClose: () => void;
...
export default function StatusAgentModal({
  agent,
  processing,
  onClose,
  onToggleStatus
}: StatusAgentModalProps) {
  return (
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div className="flex items-center">
            <div className={`p-2 rounded-lg mr-3 ${
              agent.actif ? 'bg-red-100' : 'bg-green-100'
            }`}>
              {agent.actif ? (
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-800">
                {agent.actif ? 'Désactiver' : 'Activer'} l'agent
              </h3>
              <p className="text-sm text-gray-500">Changer le statut de l'agent</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5">
          <div className="text-center mb-6">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4 ${
              agent.actif ? 'bg-red-50' : 'bg-green-50'
            }`}>
              {agent.actif ? (
          ...
          <div className="flex items-center justify-center space-x-3 pt-4 border-t border-gray-100">
            ...
            >
              ...
              <span>{processing ? 'Traitement...' : agent.actif ? 'Désactiver' : 'Activer'}</span>
            </button>
          </div>
        </div>
      </div>
