...
export default function StatusUtilisateurModal({
  utilisateur,
  processing,
  onClose,
  onToggleStatus
}: StatusUtilisateurModalProps) {
  return (
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div className="flex items-center">
            <div className={`p-2 rounded-lg mr-3 ${
              utilisateur.actif ? 'bg-red-100' : 'bg-green-100'
            }`}>
              {utilisateur.actif ? (
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-800">
                {utilisateur.actif ? 'Désactiver' : 'Activer'} l'utilisateur
              </h3>
        ...
        <div className="p-5">
          <div className="text-center mb-6">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4 ${
              utilisateur.actif ? 'bg-red-50' : 'bg-green-50'
            }`}>
              {utilisateur.actif ? (
            ...
            <p className="text-gray-600 mb-2">
              Êtes-vous sûr de vouloir {utilisateur.actif ? 'désactiver' : 'activer'} cet utilisateur ?
            </p>
            <p className="text-gray-800 font-medium">
              {utilisateur.nom_complet} <span className="text-gray-400">({utilisateur.id})</span>
            </p>
          </div>
          ...
          <div className="flex items-center justify-center space-x-3 pt-4 border-t border-gray-100">
            ...
            >
              ...
              <span>{processing ? 'Traitement...' : utilisateur.actif ? 'Désactiver' : 'Activer'}</span>
            </button>
          </div>
        </div>
      </div>
