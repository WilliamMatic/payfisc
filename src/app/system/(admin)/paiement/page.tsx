...
const PaiementPage: React.FC = () => {
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        <div className="grid grid-cols-2 gap-4 mb-6">
          ...
          <div>
            ...
            <p className="font-medium text-[#23A974]">{calculateAmount().toLocaleString()} CDF</p>
          </div>
        </div>
  ...
  // Modal de reçu - NOUVELLE VERSION MODERNISÉE
    ...
    return (
      <Portal>
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full p-8 my-8" id="receipt-content">
            {/* Filigrane */}
            <div className="fixed inset-0 flex items-center justify-center pointer-events-none opacity-10 z-0">
              <div className="text-[120px] font-bold text-gray-600 transform -rotate-45">DGRK</div>
            </div>
            
            {/* En-tête */}
            <div className="flex justify-between items-start mb-8 border-b-2 border-[#153258] pb-6">
              <div className="w-1/4">
                <div className="bg-[#153258] text-white p-3 rounded-lg text-center">
                  <div className="text-sm">RÉPUBLIQUE DÉMOCRATIQUE DU CONGO</div>
                  <div className="font-bold mt-1">DGRK</div>
                  <div className="text-xs mt-1">Direction Générale des Recettes du Kongo Central</div>
                </div>
              </div>
              
              <div className="text-center w-2/4">
                <h1 className="text-2xl font-bold text-[#153258] uppercase">NOTE DE PERCEPTION</h1>
                <p className="text-gray-600 mt-2">Référence: {paymentReference}</p>
              </div>
            ...
            {/* Informations du contribuable et détails de paiement */}
            <div className="grid grid-cols-2 gap-8 mb-8">
              <div className="bg-gray-50 p-5 rounded-lg">
                <h3 className="text-lg font-semibold text-[#153258] mb-4 border-b pb-2">
                  <i className="fas fa-user mr-2"></i>INFORMATIONS DU CONTRIBUABLE
                </h3>
                <div className="space-y-2">
                  <p><span className="font-medium text-gray-700">NIF:</span> {nif}</p>
                  <p><span className="font-medium text-gray-700">Nom:</span> {contribuable?.prenom ? `${contribuable.prenom} ${contribuable.nom}` : contribuable?.nom}</p>
                  {/* <p><span className="font-medium text-gray-700">Type:</span> {contribuable?.type}</p>
                  <p><span className="font-medium text-gray-700">Adresse:</span> {contribuable?.adresse_siege}</p> */}
                </div>
              </div>
              
              <div className="bg-gray-50 p-5 rounded-lg">
                <h3 className="text-lg font-semibold text-[#153258] mb-4 border-b pb-2">
                  <i className="fas fa-receipt mr-2"></i>DÉTAILS DE PAIEMENT
                </h3>
                <div className="space-y-2">
                  <p><span className="font-medium text-gray-700">Référence Déclaration:</span> {declarationReference}</p>
                  <p><span className="font-medium text-gray-700">Type d'impôt:</span> {selectedTax?.nom}</p>
                  <p><span className="font-medium text-gray-700">Mode de paiement:</span> {selectedMethod?.name}</p>
                  <p><span className="font-medium text-gray-700">Montant:</span> <span className="text-[#23A974] font-bold">{calculateAmount().toLocaleString()} CDF</span></p>
                  <p><span className="font-medium text-gray-700">Date:</span> {new Date().toLocaleDateString()}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-8">
              <h3 className="text-lg font-semibold text-[#153258] mb-4 border-b pb-2">
                <i className="fas fa-file-alt mr-2"></i>DÉTAILS DE LA DÉCLARATION
              </h3>
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="grid grid-cols-2 gap-4">
                  {formsData.map((form, index) => (
                    <div key={index} className="border rounded p-3">
                      <h4 className="font-medium text-[#153258] mb-2">Déclaration #{index + 1}</h4>
            ...
            <div className="bg-gray-100 p-4 rounded-lg mb-8">
              <div className="grid grid-cols-4 gap-4 text-center">
                <div>
                  <div className="text-xs text-gray-500">ID.NAT.</div>
                  <div className="text-sm font-medium">01-2345-DGRK</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">RCCM</div>
                  <div className="text-sm font-medium">CD/KK/RCCM/22-B-1234</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">IF</div>
                  <div className="text-sm font-medium">A1234567890</div>
                </div>
                <div>
                  <div className="text-xs text-gray-500">N° CNSS</div>
                  <div className="text-sm font-medium">CNSS-1234567</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex justify-between mt-12">
              <div className="text-center">
                <div className="border-t-2 border-gray-400 pt-2 w-48 mx-auto">
                  <p className="font-medium">Signature du contribuable</p>
                </div>
              </div>
              
              <div className="text-center">
                <div className="border-t-2 border-gray-400 pt-2 w-48 mx-auto">
                  <p className="font-medium">Signature et cachet de l'agent</p>
                </div>
              </div>
            </div>
            ...
            <div className="text-center text-xs text-gray-500 mt-12 pt-4 border-t">
              <p>Direction Générale des Recettes du Kongo Central - Avenue de la Nation N°123, Matadi</p>
              <p>Tél: +243 999 999 999 - Email: info@dgrk.cd - Site web: www.dgrk.cd</p>
              <p className="mt-2">Ce document est une preuve légale de votre déclaration et paiement. Conservez-le pour vos archives.</p>
            </div>
