...
interface StatusSiteModalProps {
  site: SiteType;
  processing: boolean;
  onClose: () => void;
...
export default function StatusSiteModal({
  site,
  processing,
  onClose,
  onToggleStatus
}: StatusSiteModalProps) {
  return (
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div className="flex items-center">
            <div className={`p-2 rounded-lg mr-3 ${
              site.actif ? 'bg-red-100' : 'bg-green-100'
            }`}>
              {site.actif ? (
        ...
        <div className="p-5">
          <div className="text-center mb-6">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4 ${
              site.actif ? 'bg-red-50' : 'bg-green-50'
            }`}>
              {site.actif ? (
            ...
            <p className="text-gray-600 mb-2">
              Êtes-vous sûr de vouloir {site.actif ? 'désactiver' : 'activer'} ce site ?
            </p>
            <p className="text-gray-800 font-medium">
              {site.nom} <span className="text-gray-400">({site.code})</span>
            </p>
          </div>
          ...
          <div className="flex items-center justify-center space-x-3 pt-4 border-t border-gray-100">
            ...
            >
              ...
              <span>{processing ? 'Traitement...' : site.actif ? 'Désactiver' : 'Activer'}</span>
            </button>
          </div>
        </div>
      </div>
