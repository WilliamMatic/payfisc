...
interface UsagesHeaderProps {
  searchTerm: string;
  onSearchChange: (value: string) => void;
  onAddClick: () => void;
}

export default function UsagesHeader({ searchTerm, onSearchChange, onAddClick }: UsagesHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
      <div className="flex items-center">
        ...
        <div>
          <h2 className="text-xl font-semibold text-gray-800">Usages d'Engins</h2>
          <p className="text-sm text-gray-500">Gérez les usages des types d'engins (taxi, personnel, etc.)</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
        ...
        >
          ...
          <span className="font-medium text-sm">Nouvel Usage</span>
        </button>
      </div>
    </div>
  );
