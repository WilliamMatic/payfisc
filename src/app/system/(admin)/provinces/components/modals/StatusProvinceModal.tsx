...
interface StatusProvinceModalProps {
...
export default function StatusProvinceModal({
  province,
  processing,
  onClose,
  onToggleStatus
}: StatusProvinceModalProps) {
  return (
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div className="flex items-center">
            <div className={`p-2 rounded-lg mr-3 ${
              province.actif ? 'bg-red-100' : 'bg-green-100'
            }`}>
              {province.actif ? (
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-800">
                {province.actif ? 'Désactiver' : 'Activer'} la province
              </h3>
        ...
        <div className="p-5">
          <div className="text-center mb-6">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4 ${
              province.actif ? 'bg-red-50' : 'bg-green-50'
            }`}>
              {province.actif ? (
            ...
            <p className="text-gray-600 mb-2">
              Êtes-vous sûr de vouloir {province.actif ? 'désactiver' : 'activer'} cette province ?
            </p>
            <p className="text-gray-800 font-medium">
              {province.nom} <span className="text-gray-400">({province.code})</span>
            </p>
          </div>
          ...
          <div className="flex items-center justify-center space-x-3 pt-4 border-t border-gray-100">
            ...
            >
              ...
              <span>{processing ? 'Traitement...' : province.actif ? 'Désactiver' : 'Activer'}</span>
            </button>
          </div>
        </div>
      </div>
