// app/type-engins/components/TypeEnginsHeader.tsx
import { Settings, Search, Plus } from 'lucide-react';

interface TypeEnginsHeaderProps {
  searchTerm: string;
  onSearchChange: (value: string) => void;
  onAddClick: () => void;
}

export default function TypeEnginsHeader({ searchTerm, onSearchChange, onAddClick }: TypeEnginsHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
      <div className="flex items-center">
        ...
        <div>
          <h2 className="text-xl font-semibold text-gray-800">Types d'Engins</h2>
          <p className="text-sm text-gray-500">Gérez les types d'engins de votre application</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
        <div className="relative flex-1 sm:w-64">
          ...
          <input
            type="text"
            placeholder="Rechercher un type d'engin..."
            value={searchTerm}
