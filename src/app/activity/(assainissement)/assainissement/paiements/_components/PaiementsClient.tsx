...
import {
  getPaiements, enregistrerPaiement, getFactures, getRepartitionPaiement, getAxes,
} from "@/services/assainissement/assainissementService";
import { Paiement, Facture, Repartition, Axe, Pagination } from "@/services/assainissement/types";
import {
  Search, ChevronLeft, ChevronRight, Filter, Calendar, Printer,
  X, Plus, Eye, DollarSign,
} from "lucide-react";
...
export default function PaiementsClient() {
  ...
  const handlePay = async () => {
    ...
    const res = await enregistrerPaiement({
      facture_id: Number(payForm.facture_id),
      contribuable_id: facture.contribuable_id,
  ...
  const handlePrint = () => {
    ...
    w.document.write(`<!DOCTYPE html><html><head><title>Reçu</title><style>
      ...
      * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Arial, sans-serif; }
      ...
      .header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 3px solid #23A974; padding-bottom: 20px; margin-bottom: 20px; }
      .title { font-size: 28px; font-weight: bold; color: #23A974; }
      ...
      th { background: #23A974; color: white; padding: 10px 12px; font-size: 12px; text-align: left; }
      td { padding: 10px 12px; border-bottom: 1px solid #e5e7eb; font-size: 13px; }
      .total-row { background: #f0fdf4; font-weight: bold; }
      .total-amount { font-size: 18px; color: #23A974; }
      .rep-title { font-size: 14px; font-weight: bold; color: #153258; margin: 15px 0 8px; }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button onClick={openPayModal} className="flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#153258] to-[#23A974] text-white rounded-lg hover:shadow-lg transition-all text-sm font-medium">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {paiements.length === 0 ? (
                  <tr><td colSpan={7} className="text-center py-8 text-gray-400">Aucun paiement trouvé</td></tr>
                ) : paiements.map((p) => (
                  <tr key={p.id} className="border-t border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                    ...
                    <td className="px-4 py-3">
                      <p className="font-medium text-gray-900 dark:text-white">{p.contribuable_nom} {p.contribuable_prenom}</p>
                      <p className="text-xs text-gray-500">{p.commune_nom}</p>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-full">{MODE_LABELS[p.mode_paiement] || p.mode_paiement}</span>
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-600 dark:text-gray-400">{new Date(p.date_paiement).toLocaleString("fr-FR")}</td>
                    <td className="px-4 py-3 text-right">
                      <button onClick={() => openReceipt(p)} className="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg" title="Reçu"><Eye className="w-4 h-4 text-blue-500" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-lg">
            <div className="flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2"><DollarSign className="w-5 h-5 text-[#23A974]" /> Enregistrer un paiement</h3>
              <button onClick={() => setShowPayModal(false)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg"><X className="w-5 h-5 text-gray-400" /></button>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Facture impayée *</label>
                <select value={payForm.facture_id} onChange={(e) => setPayForm({ ...payForm, facture_id: e.target.value })}
                  ...
                  {facturesImpayees.map((f) => (
                    <option key={f.id} value={f.id}>{f.reference} — {f.contribuable_nom} {f.contribuable_prenom} — {formatMontant(Number(f.montant))}</option>
                  ))}
                </select>
              </div>
              {selectedFacture && (
                <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg text-sm">
                  <p className="font-medium text-blue-700 dark:text-blue-300">Montant: {formatMontant(Number(selectedFacture.montant))}</p>
                  <p className="text-xs text-blue-600 dark:text-blue-400">{selectedFacture.type_taxe_nom} · {selectedFacture.commune_nom}</p>
                </div>
              )}
              <div>
              ...
              {payForm.mode_paiement === "mobile_money" && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Numéro mobile *</label>
                  <input type="tel" value={payForm.numero_mobile} onChange={(e) => setPayForm({ ...payForm, numero_mobile: e.target.value })}
                    placeholder="+243..." className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-[#153258] outline-none" />
                </div>
              )}
              {payForm.mode_paiement === "banque" && (
                <>
                  <div>
                    ...
                    <input type="text" value={payForm.nom_banque} onChange={(e) => setPayForm({ ...payForm, nom_banque: e.target.value })}
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-[#153258] outline-none" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">N° carte</label>
                      <input type="text" value={payForm.numero_carte} onChange={(e) => setPayForm({ ...payForm, numero_carte: e.target.value })}
                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-[#153258] outline-none" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Titulaire</label>
                      <input type="text" value={payForm.titulaire_carte} onChange={(e) => setPayForm({ ...payForm, titulaire_carte: e.target.value })}
                        className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-[#153258] outline-none" />
                    </div>
                  </div>
                </>
            ...
            <div className="flex items-center justify-end gap-3 p-5 border-t border-gray-200 dark:border-gray-700">
              ...
              <button onClick={handlePay} disabled={saving || !payForm.facture_id}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden">
            <div className="flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handlePrint} className="flex items-center gap-2 px-3 py-2 bg-[#23A974] text-white rounded-lg text-sm hover:bg-[#23A974]/90"><Printer className="w-4 h-4" /> Imprimer A4</button>
                <button onClick={() => { setShowReceipt(null); setRepartitions([]); }} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg"><X className="w-5 h-5 text-gray-400" /></button>
              </div>
            </div>
            <div className="p-5 overflow-y-auto max-h-[70vh]">
              <div ref={printRef}>
                <div className="header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #23A974", paddingBottom: 20, marginBottom: 20 }}>
                  <div>
                    <div className="title" style={{ fontSize: 28, fontWeight: "bold", color: "#23A974" }}>REÇU DE PAIEMENT</div>
                    <div className="subtitle" style={{ fontSize: 12, color: "#666" }}>Taxe d&apos;assainissement</div>
                    <div style={{ fontSize: 14, fontWeight: 600, marginTop: 8, color: "#153258" }}>{showReceipt.reference}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 16, fontWeight: "bold", color: "#153258" }}>PayFisc</div>
                    <div style={{ fontSize: 11, color: "#999" }}>Date: {new Date(showReceipt.date_paiement).toLocaleString("fr-FR")}</div>
                  </div>
                </div>
                ...
                <div className="info-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
                  <div className="info-box" style={{ background: "#f8f9fa", borderRadius: 8, padding: 15 }}>
                    ...
                    <p style={{ fontSize: 13, lineHeight: 1.6 }}>
                      <strong>{showReceipt.contribuable_nom} {showReceipt.contribuable_prenom}</strong><br />
                      {showReceipt.nom_etablissement && <>{showReceipt.nom_etablissement}<br /></>}
                      Réf: {showReceipt.contribuable_ref}<br />
                      {showReceipt.contribuable_tel && <>Tél: {showReceipt.contribuable_tel}<br /></>}
                      {showReceipt.commune_nom && <>Axe: {showReceipt.commune_nom}<br /></>}
                      {showReceipt.quartier_nom && <>Quartier: {showReceipt.quartier_nom}</>}
                    </p>
                  </div>
                  <div className="info-box" style={{ background: "#f8f9fa", borderRadius: 8, padding: 15 }}>
                    <h4 style={{ fontSize: 11, textTransform: "uppercase", color: "#999", marginBottom: 8, letterSpacing: 1 }}>Paiement</h4>
                    <p style={{ fontSize: 13, lineHeight: 1.6 }}>
                      Facture: {showReceipt.facture_ref}<br />
                      Période: {showReceipt.periode_debut ? new Date(showReceipt.periode_debut).toLocaleDateString("fr-FR") : "—"} — {showReceipt.periode_fin ? new Date(showReceipt.periode_fin).toLocaleDateString("fr-FR") : "—"}<br />
                      Mode: {MODE_LABELS[showReceipt.mode_paiement] || showReceipt.mode_paiement}<br />
                      {showReceipt.numero_mobile && <>N° mobile: {showReceipt.numero_mobile}<br /></>}
                      {showReceipt.nom_banque && <>Banque: {showReceipt.nom_banque}</>}
                    </p>
                  </div>
                </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", margin: "20px 0" }}>
                  <thead>
                    <tr>
                      <th style={{ background: "#23A974", color: "white", padding: "10px 12px", fontSize: 12, textAlign: "left" }}>Description</th>
                      <th style={{ background: "#23A974", color: "white", padding: "10px 12px", fontSize: 12, textAlign: "right" }}>Montant</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr>
                      <td style={{ padding: "10px 12px", borderBottom: "1px solid #e5e7eb", fontSize: 13 }}>Taxe d&apos;assainissement — {showReceipt.type_taxe_nom}</td>
                      <td style={{ padding: "10px 12px", borderBottom: "1px solid #e5e7eb", fontSize: 13, textAlign: "right" }}>{formatMontant(Number(showReceipt.montant))}</td>
                    </tr>
                    <tr className="total-row" style={{ background: "#f0fdf4" }}>
                      <td style={{ padding: "12px", fontSize: 14, fontWeight: "bold" }}>TOTAL PAYÉ</td>
                      <td className="total-amount" style={{ padding: "12px", textAlign: "right", fontSize: 18, color: "#23A974", fontWeight: "bold" }}>{formatMontant(Number(showReceipt.montant))}</td>
                    </tr>
                  </tbody>
                </table>
                ...
                {repartitions.length > 0 && (
                  <>
                    <div className="rep-title" style={{ fontSize: 14, fontWeight: "bold", color: "#153258", margin: "15px 0 8px" }}>Répartition du paiement</div>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      ...
                      <tbody>
                        {repartitions.map((r) => (
                          <tr key={r.id}>
                            <td style={{ padding: "8px 12px", borderBottom: "1px solid #e5e7eb", fontSize: 12 }}>{r.beneficiaire_nom}</td>
                            <td style={{ padding: "8px 12px", borderBottom: "1px solid #e5e7eb", fontSize: 12 }}>{r.numero_compte || "—"}</td>
                            <td style={{ padding: "8px 12px", borderBottom: "1px solid #e5e7eb", fontSize: 12, textAlign: "right", fontWeight: "bold" }}>{formatMontant(Number(r.montant))}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 20 }}>
                  <div style={{ textAlign: "center" }}>
                    <QRCodeSVG value={`RECU:${showReceipt.reference}|MONTANT:${showReceipt.montant}|DATE:${showReceipt.date_paiement}`} size={80} />
                    <p style={{ fontSize: 9, color: "#999", marginTop: 4 }}>{showReceipt.reference}</p>
                  </div>
                </div>

                <div className="footer" style={{ textAlign: "center", marginTop: 30, paddingTop: 15, borderTop: "1px solid #e5e7eb", fontSize: 11, color: "#999" }}>
                  PayFisc — Système de gestion fiscale · Reçu de paiement — Taxe d&apos;assainissement · Document généré automatiquement
                </div>
              </div>
            </div>
          </div>
