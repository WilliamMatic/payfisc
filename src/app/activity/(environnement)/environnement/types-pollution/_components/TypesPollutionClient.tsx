...
import { getTypesPollution, addTypePollution, updateTypePollution, deleteTypePollution } from "@/services/environnement/environnementService";
import { TypePollution } from "@/services/environnement/types";
import { Plus, Search, Edit2, Trash2, X } from "lucide-react";

export default function TypesPollutionClient() {
  const { utilisateur } = useAuth();
  const [items, setItems] = useState<TypePollution[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [editItem, setEditItem] = useState<TypePollution | null>(null);
  const [deleteItem, setDeleteItem] = useState<TypePollution | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    const res = await getTypesPollution(utilisateur.site_id);
  ...
  const openEdit = (t: TypePollution) => { setEditItem(t); setForm({ nom: t.nom, code: t.code, description: t.description || "" }); setShowModal(true); };
  const handleSave = async () => {
    if (!form.nom.trim() || !form.code.trim() || !utilisateur?.site_id) return;
    setSaving(true);
    const data = { ...form, site_id: utilisateur.site_id };
    const res = editItem ? await updateTypePollution({ ...data, id: editItem.id }) : await addTypePollution(data);
    if (res.status === "success") { setShowModal(false); load(); }
    setSaving(false);
  };
  const handleDelete = async () => { if (!deleteItem) return; setSaving(true); const res = await deleteTypePollution(deleteItem.id); if (res.status === "success") { setDeleteItem(null); load(); } setSaving(false); };

  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white">Types de pollution</h1><p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Gestion des types de pollution environnementale</p></div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          <tbody>{filtered.length === 0 ? <tr><td colSpan={4} className="text-center py-8 text-gray-400">Aucun type</td></tr> : filtered.map(t => (
            <tr key={t.id} className="border-t border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/30">
              ...
              <td className="px-4 py-3"><span className="px-2 py-1 bg-orange-50 dark:bg-orange-900/20 text-orange-600 rounded text-xs font-mono">{t.code}</span></td>
