...
import FoncierSidebar from "./components/FoncierSidebar";
import Header from "../(users)/components/Header";
import { useAuth } from "@/contexts/AuthContext";
import { useTheme } from "@/contexts/ThemeContext";
import { useRouter } from "next/navigation";

export default function FoncierLayoutClient({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isLoading && isAuthenticated) {
      if (userType === "agent") router.push("/system/login");
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100 dark:bg-gray-900">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#153258]" />
      </div>
    );
  }

  if (!isAuthenticated || userType === "agent") return null;

  return (
    <div className={`min-h-screen transition-all duration-500 ${theme === "dark" ? "dark" : ""}`}>
      <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
        <FoncierSidebar isSidebarOpen={isSidebarOpen} setIsSidebarOpen={setIsSidebarOpen} />
        {isSidebarOpen && (
          <div
