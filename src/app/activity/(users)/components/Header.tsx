...
export default function Header({ 
  ...
  const getDisplayInfo = () => {
    ...
    if (userType === 'utilisateur' && utilisateur) {
      const initials = utilisateur.nom_complet.split(' ')
        .map(n => n[0])
        .join('')
        .toUpperCase()
        .substring(0, 2);
      ...
      <div className="px-5 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="relative" ref={userMenuRef}>
            ...
            >
              <div className={`w-8 h-8 bg-gradient-to-r ${displayInfo.color} rounded-full flex items-center justify-center text-white text-sm font-medium`}>
                {displayInfo.initial}
              </div>
            ...
            {showUserMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-100 z-50">
                <div className="p-3 border-b border-gray-100">
                  <p className="text-sm font-medium text-gray-800">{displayInfo.name}</p>
                  <p className="text-xs text-gray-500">{displayInfo.detail}</p>
                  <p className="text-xs text-gray-400 mt-1">{displayInfo.badge}</p>
                </div>
                <button 
                  onClick={onLogout}
