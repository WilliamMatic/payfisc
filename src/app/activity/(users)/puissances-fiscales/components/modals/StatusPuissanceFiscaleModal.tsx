...
interface StatusPuissanceFiscaleModalProps {
...
export default function StatusPuissanceFiscaleModal({
  puissance,
  processing,
  onClose,
  onToggleStatus
}: StatusPuissanceFiscaleModalProps) {
  return (
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div className="flex items-center">
            <div className={`p-2 rounded-lg mr-3 ${
              puissance.actif ? 'bg-red-100' : 'bg-green-100'
            }`}>
              {puissance.actif ? (
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-800">
                {puissance.actif ? 'Désactiver' : 'Activer'} la puissance
              </h3>
        ...
        <div className="p-5">
          <div className="text-center mb-6">
            <div className={`w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-4 ${
              puissance.actif ? 'bg-red-50' : 'bg-green-50'
            }`}>
              {puissance.actif ? (
            ...
            <p className="text-gray-600 mb-2">
              Êtes-vous sûr de vouloir {puissance.actif ? 'désactiver' : 'activer'} cette puissance fiscale ?
            </p>
            <p className="text-gray-800 font-medium">
              {puissance.libelle} <span className="text-gray-400">({puissance.valeur} CV)</span>
            </p>
            <p className="text-gray-600 text-sm mt-1">
              Type: {puissance.type_engin_libelle}
            </p>
          </div>
          ...
          <div className="flex items-center justify-center space-x-3 pt-4 border-t border-gray-100">
            ...
            >
              ...
              <span>{processing ? 'Traitement...' : puissance.actif ? 'Désactiver' : 'Activer'}</span>
            </button>
          </div>
        </div>
      </div>
