import { Gauge, Search, Plus } from 'lucide-react';

interface PuissanceFiscaleHeaderProps {
  searchTerm: string;
  onSearchChange: (value: string) => void;
  onAddClick: () => void;
}

export default function PuissanceFiscaleHeader({ searchTerm, onSearchChange, onAddClick }: PuissanceFiscaleHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
      ...
      <div className="flex items-center">
        ...
        <div>
          <h2 className="text-xl font-semibold text-gray-800">Puissances Fiscales</h2>
          <p className="text-sm text-gray-500">Gérez les puissances fiscales des engins</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
        <div className="relative flex-1 sm:w-56">
          ...
          <input
            type="text"
            placeholder="Rechercher une puissance..."
            value={searchTerm}
        ...
        >
          ...
          <span className="font-medium text-sm">Nouvelle Puissance</span>
        </button>
      </div>
    </div>
  );
