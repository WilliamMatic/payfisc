// app/energies/components/EnergiesHeader.tsx
import { Zap, Search, Plus } from 'lucide-react';

interface EnergiesHeaderProps {
  searchTerm: string;
  onSearchChange: (value: string) => void;
  onAddClick: () => void;
}

export default function EnergiesHeader({ searchTerm, onSearchChange, onAddClick }: EnergiesHeaderProps) {
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
      <div className="flex items-center">
        ...
        <div>
          <h2 className="text-xl font-semibold text-gray-800">Énergies</h2>
          <p className="text-sm text-gray-500">Gérez les types d'énergie des engins</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
        ...
        >
          ...
          <span className="font-medium text-sm">Nouvelle Énergie</span>
        </button>
      </div>
    </div>
  );
