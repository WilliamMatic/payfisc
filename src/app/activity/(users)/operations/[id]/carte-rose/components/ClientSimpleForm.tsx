...
// Interface pour les données de la fiche supplémentaire
...
export default function ClientSimpleForm({
  ...
  const handleRetourVerification = () => {
    reinitialiserFormulaire();
    setEtapeActuelle("verification");
    showMessage(
      "info",
      "Retour à la vérification",
      "Le formulaire a été réinitialisé. Vous allez être redirigé vers l'étape de vérification.",
    );
  };
  ...
  const handleInputChange = (field: keyof FormData, value: string) => {
    ...
    // Réinitialiser la marque et le modèle si le type d'engin change
    if (field === "typeEngin") {
      setFormData((prev) => ({
        ...prev,
        marque: "",
  ...
  const handleAddCouleur = async (nom: string, codeHex: string) => {
    ...
    try {
      ...
      if (response.status === "success") {
        // Construire l'objet couleur depuis la réponse OU en local
        ...
        showMessage(
          "success",
          "Couleur ajoutée",
          `La couleur "${nom}" a été ajoutée et sélectionnée.`,
        );
      } else {
        showMessage(
          "error",
      ...
      showMessage(
        "error",
        "Erreur réseau",
        "Erreur réseau lors de l'ajout de la couleur",
      );
    } finally {
      ...
      verificationTelephoneTimeoutRef.current = setTimeout(async () => {
        ...
        try {
          ...
          showMessage(
            ...
            "Erreur lors de la vérification du téléphone",
          );
        } finally {
    ...
    try {
      ...
      if (result.status === "success" && result.data) {
        ...
        showMessage(
          "error",
          "Erreur de création",
          result.message || "Erreur lors de la création du modèle",
        );
      }
    } catch (error) {
      ...
      showMessage(
        "error",
        "Erreur réseau",
        "Erreur réseau lors de la création du modèle",
      );
    } finally {
      setLoading((prev) => ({ ...prev, rechercheModeles: false }));
    }
    ...
    try {
      ...
      if (result.status === "success" && result.data) {
        ...
        showMessage(
          "success",
          "Puissance créée",
          "La puissance fiscale a été créée avec succès",
        );
      } else {
        showMessage(
          "error",
          "Erreur de création",
          result.message ||
            "Erreur lors de la création de la puissance fiscale",
        );
      }
    } catch (error) {
      ...
      showMessage(
        "error",
        "Erreur réseau",
        "Erreur réseau lors de la création de la puissance fiscale",
      );
    } finally {
      setLoading((prev) => ({ ...prev, recherchePuissances: false }));
    }
  ...
  const handleVerification = async () => {
    ...
    try {
      ...
      const result: CarteRoseResponse =
        await verifierPlaqueTelephone(verificationData);
  ...
  const passerAuFormulaire = () => {
    ...
    showMessage(
      "info",
      "Formulaire ouvert",
      "Vous pouvez maintenant compléter les informations du formulaire.",
    );
  };
  ...
  const reinitialiserChampsVerification = () => {
    ...
    showMessage(
      ...
      "Les champs de vérification ont été réinitialisés.",
    );
  };
  ...
  // Gestion de la soumission des informations supplémentaires pour la fiche
  const handleSubmitSupplementaire = (suppData: FicheSupplementaire) => {
    ...
    showMessage(
      "success",
      "Fiche générée",
      "La fiche d'identification a été générée avec succès.",
    );
  };
  ...
  const handleImprimer = () => {
    ...
    showMessage(
      "info",
      "Impression",
      "Préparation de l'impression de la carte rose...",
    );
  };
  ...
  const handlePrintClose = () => {
    ...
    showMessage(
      ...
      "Le formulaire a été réinitialisé. Vous pouvez créer une nouvelle carte rose.",
    );
  };

  // Section pour le champ Marque avec auto-complétion
      ...
      {showMarquesSuggestions &&
        ...
        formData.marque.length >= 2 && (
          <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg p-3 text-sm text-gray-600">
            Aucune marque trouvée. La marque sera créée automatiquement lors de
            la soumission.
          </div>
        )}

      {errors.marque && (
        <p className="text-red-600 text-sm mt-1 font-medium">{errors.marque}</p>
      )}
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
      <div className="flex items-center space-x-3 mb-5 pb-4 border-b border-gray-100">
        <div className="w-8 h-8 bg-[#2D5B7A]/10 rounded-lg flex items-center justify-center flex-shrink-0">
          <CheckCircle className="w-4 h-4 text-[#2D5B7A]" />
        </div>
        <div>
  ...
  const renderEtapeFormulaire = () => (
    <form onSubmit={handleSubmitForm} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <div className="text-gray-700 font-medium">
              {particulierInfo?.telephone || "Non fourni"}
            </div>
          </div>
          <div>
      ...
      <div className="flex items-center justify-between">
        <button
          type="button"
          onClick={handleRetourVerification}
        ...
        <button
          type="submit"
          className="flex items-center space-x-2 px-6 py-3 bg-[#2D5B7A] text-white rounded-xl hover:bg-[#244D68] transition-colors font-semibold"
        >
  ...
  // Rendu de l'étape prévisualisation avec bouton pour la fiche
