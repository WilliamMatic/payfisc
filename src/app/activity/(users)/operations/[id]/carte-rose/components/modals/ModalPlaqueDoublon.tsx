"use client";
import { AlertTriangle, Car, MapPin, User, UserCheck, X } from "lucide-react";
import type { CarteExistanteDetails } from "@/services/carte-rose/carteRoseService";

interface ModalPlaqueDoublonProps {
  isOpen: boolean;
...
export default function ModalPlaqueDoublon({
  isOpen,
  details,
  onClose,
}: ModalPlaqueDoublonProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        ...
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 bg-orange-600 rounded-t-2xl shrink-0">
          <div className="flex items-center gap-3">
            <AlertTriangle className="w-6 h-6 text-white" />
            <div>
              <h3 className="font-bold text-white text-lg leading-tight">
                Plaque Déjà Enregistrée
              </h3>
              {details?.numero_plaque && (
                <p className="text-orange-100 text-sm font-mono tracking-widest">
                  {details.numero_plaque}
                </p>
              )}
            </div>
        ...
        <div className="overflow-y-auto p-6 space-y-5">
          ...
          <div className="bg-orange-50 border border-orange-200 rounded-lg px-4 py-3 text-sm text-orange-800">
            Cette plaque est déjà enregistrée dans cette province. Voici les informations du
            dossier existant. Impossible de créer un doublon.
          </div>
              ...
              {details.engin_id && (
                <div className="rounded-xl border border-green-100 overflow-hidden">
                  ...
                  <div className="px-4 py-3">
                    ...
                    <Row label="Date enreg."       value={details.engin_date_creation} />
                  </div>
                </div>
              )}

              {/* Site + Agent */}
              {(details.site_nom || details.agent_nom) && (
        ...
        <div className="shrink-0 px-6 py-4 border-t border-gray-100 flex justify-end">
          <button
            onClick={onClose}
            className="px-6 py-2.5 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors font-medium text-sm"
          >
            Compris, recommencer
          </button>
        </div>
