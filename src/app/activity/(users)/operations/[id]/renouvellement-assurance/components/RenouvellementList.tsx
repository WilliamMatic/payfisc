...
import AssuranceDetailsModal from "./AssuranceDetailsModal";
import RenouvellementConfirmModal from "./RenouvellementConfirmModal";
import SuccessModal from "./SuccessModal";
import { getAssurancesARenouveler, renouvelerAssurance } from "@/services/assurance-moto/assuranceMotoService";
import { useAuth } from "@/contexts/AuthContext";
import { Assurance, Impot } from "./types";
...
export default function RenouvellementList({ impot }: RenouvellementListProps) {
  ...
  const [filteredAssurances, setFilteredAssurances] = useState<Assurance[]>([]);
  ...
  const [selectedAssurance, setSelectedAssurance] = useState<Assurance | null>(
    null,
  );
  ...
  // Charger les données
  const loadAssurances = async (statut?: string, recherche?: string) => {
    setIsLoading(true);
    try {
      const filtres: {
        seuil_jours?: number;
      ...
      const result = await getAssurancesARenouveler(filtres);
      if (result.status === 'success' && result.data) {
        const mapped: Assurance[] = result.data.map((item: any) => ({
          id: item.id,
          assujetti: {
          ...
          mode_paiement: '',
          reference_paiement: item.code_assurance,
          jours_restants: item.jours_restants,
        }));
        setAssurances(mapped);
        setFilteredAssurances(mapped);
      } else {
        setAssurances([]);
        setFilteredAssurances([]);
      }
    } catch {
      setAssurances([]);
      setFilteredAssurances([]);
  ...
  const handleRenew = (assurance: Assurance) => {
    setSelectedAssurance(assurance);
  ...
  const handleRenouvellementSuccess = (data: any) => {
    ...
    setRenouvellementData({
      ...
      engin: {
        numero_plaque: selectedAssurance?.engin?.numero_plaque,
      },
      assujetti: {
        nom_complet: selectedAssurance?.assujetti?.nom_complet,
      },
    });
  ...
  const getStatutBadge = (assurance: Assurance) => {
  ...
  const stats = {
    ...
    expirees: assurances.filter((a) => new Date(a.date_expiration) < new Date())
      .length,
    proches: assurances.filter((a) => {
      const dateExpiration = new Date(a.date_expiration);
      const diffJours = Math.ceil(
              ...
              <tbody className="divide-y divide-gray-200">
                {isLoading ? (
                  ...
                  filteredAssurances.map((assurance) => {
                    const statut = getStatutBadge(assurance);
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-600">
                            {new Date(assurance.date_souscription).toLocaleDateString(
                              "fr-FR",
                            )}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right space-x-2">
                          <button
                            onClick={() => handleViewDetails(assurance)}
