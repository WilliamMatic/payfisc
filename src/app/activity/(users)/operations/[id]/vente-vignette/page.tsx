...
import VignetteSearch from "./components/VignetteSearch";
...
export default async function VenteVignettePage({ params }: PageProps) {
  const { id } = await params;

  // Récupérer l'impôt par son ID
  ...
  // Récupérer le prix depuis l'impôt
  const prix = Number(impot.prix) || 0;

  // Récupérer le taux de change actif pour cet impôt
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-blue-50/30 py-8">
      <div className="container mx-auto px-4 max-w-7xl">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Recherchez un véhicule par sa plaque minéralogique
              </p>
            </div>
        ...
        <VignetteSearch impot={impot} prix={prix} tauxCdf={tauxCdf} />
      </div>
    </div>
  );
}
