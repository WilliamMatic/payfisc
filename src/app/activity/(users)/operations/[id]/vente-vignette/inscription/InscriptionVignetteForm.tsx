...
import {
  getBeneficiairesImpot,
  type BeneficiaireImpot,
  type BeneficiaireParProvince,
...
interface InscriptionVignetteFormProps {
  plaque: string;
  operationId: string;
}

// ─── Modal Ajout Couleur (identique carte-rose) ───
...
// ─── Modal Messages (identique carte-rose) ───
...
export default function InscriptionVignetteForm({
  plaque,
  operationId,
}: InscriptionVignetteFormProps) {
  ...
  const [beneficiaires, setBeneficiaires] = useState<BeneficiaireParProvince[]>([]);
  const [numeroVignette, setNumeroVignette] = useState("");
  const [vignetteError, setVignetteError] = useState("");
  ...
  // ─── Loading states (identique carte-rose) ───
  ...
  const repartition = useMemo(() => {
    if (!beneficiaires.length || !prix) return [];
    const allBeneficiaires: (BeneficiaireImpot & { montant: number })[] = [];
    let totalReparti = 0;
    for (const province of beneficiaires) {
      for (const b of province.beneficiaires) {
        let montant = 0;
        if (b.type_part === "pourcentage") {
          montant = (prix * b.valeur_part) / 100;
        } else {
          montant = Math.min(b.valeur_part, prix);
        }
        montant = Math.round(montant * 100) / 100;
        totalReparti += montant;
        allBeneficiaires.push({ ...b, montant });
      }
    }
    if (totalReparti > prix && allBeneficiaires.length > 0) {
      const diff = totalReparti - prix;
      allBeneficiaires[allBeneficiaires.length - 1].montant = Math.max(
        0,
        allBeneficiaires[allBeneficiaires.length - 1].montant - diff,
      );
    }
    return allBeneficiaires;
  }, [beneficiaires, prix]);

  // ─── Chargement initial (identique carte-rose + impôt/taux/bénéficiaires) ───

  useEffect(() => {
    const loadData = async () => {
      setIsLoadingData(true);
      try {
        const [impotRes, typeEnginsRes, energiesRes, couleursRes, usagesRes, puissancesRes] =
          await Promise.all([
        ...
        if (impotRes.status === "success" && impotRes.data) {
          ...
          const prixVal = Number(impotRes.data.prix) || 0;
          setPrix(prixVal);

          const [tauxRes, benefRes] = await Promise.all([
            getTauxActif({ impot_id: impotRes.data.id }),
            getBeneficiairesImpot(impotRes.data.id),
          ]);
          if (tauxRes.status === "success" && tauxRes.data) {
            setTauxCdf(Number(tauxRes.data.valeur) || 0);
          }
          if (benefRes.status === "success" && benefRes.data) {
        ...
        if (puissancesRes.status === "success") setPuissancesFiscales(puissancesRes.data || []);
      } catch (error) {
  ...
  // ─── Recherche automatique des marques (identique carte-rose) ───
  ...
  // ─── Gestion des inputs (identique carte-rose) ───
  ...
  // ─── Sélection marque (identique carte-rose) ───
  ...
  // ─── Couleur : ajout modal (identique carte-rose) ───
  ...
  // ─── Vérification téléphone temps réel (identique carte-rose) ───
  ...
  // ─── Recherche modèles (identique carte-rose) ───
  ...
  // ─── Recherche puissance fiscale (identique carte-rose) ───
  ...
  // ─── Validation (identique carte-rose) ───
  ...
  const validateForm = (): boolean => {
    ...
    if (!numeroVignette.trim()) {
      setVignetteError("Le numéro de vignette physique est obligatoire");
    } else {
      setVignetteError("");
    }
    if (formData.anneeFabrication && formData.anneeCirculation) {
    ...
    setErrors(newErrors);
    if (Object.keys(newErrors).length > 0 || !numeroVignette.trim()) {
  ...
  // ─── Soumission : d'abord afficher le récap ───
  ...
  const handleConfirmerSoumission = async () => {
    ...
    try {
      ...
      const inscriptionResult = await inscrireAssujetti({
        ...
        impot_id: impot ? String(impot.id) : undefined,
      });
      ...
      const paiementResult = await enregistrerPaiementVignette({
        engin_id: enginId,
        particulier_id: particulierId,
        montant: prix,
        montant_initial: prix,
      ...
      if (paiementResult.status === "success") {
        ...
        showMessage("error", "Erreur de paiement", paiementResult.message || "Erreur lors du paiement");
      }
    } catch (err) {
  ...
  // ─── Section Marque auto-complétion (identique carte-rose) ───
  ...
  // ─── Section Couleur select + bouton + (identique carte-rose) ───
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <div className="flex items-center space-x-3 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-600 text-sm">
              La vignette a été enregistrée pour la plaque <strong>{plaque}</strong>.
            </p>
          </div>
        </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-6">
          <div className="text-center">
            ...
            <h3 className="text-xl font-bold text-green-800 mb-2">Vignette Validée</h3>
            <p className="text-green-700">
              La vignette pour le véhicule <strong>{plaque}</strong> a été délivrée.
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-lg p-4 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <div className="font-bold text-green-600">{prix}$</div>
            </div>
            <div>
        ...
        <div className="flex justify-center space-x-4">
          ...
          <button
            onClick={() => router.push(`/activity/operations/${operationId}/vente-vignette`)}
  ...
  return (
    <>
              ...
              <div className="space-y-6 mb-6">
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="font-bold text-green-700">{prix}$</div>
                    </div>
                  </div>
                </div>

                {/* Répartition dans le récap */}
                {repartition.length > 0 && (
                  <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                    <h4 className="font-semibold text-purple-800 mb-3">Répartition Bénéficiaires</h4>
                    <div className="space-y-2">
                      {repartition.map((b, i) => (
                        <div key={i} className="flex justify-between text-sm">
                          <span className="text-purple-700">{b.nom} ({b.type_part === "pourcentage" ? `${b.valeur_part}%` : `${b.valeur_part}$ fixe`})</span>
                          <span className="font-bold text-purple-800">{b.montant.toFixed(2)}$</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-yellow-50 rounded-lg p-4 border border-yellow-200">
                  ...
                  <p className="text-yellow-700 text-sm">
                    Voulez-vous confirmer l&apos;inscription et le paiement de la vignette ?
                ...
                >
                  {isSubmitting ? (
                    ...
                    <>
                      ...
                      <span>Confirmer le Paiement ({prix}$)</span>
                    </>
                  )}
                </button>
              </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {prix > 0 && (
            <div className="text-right">
              ...
              <div className="text-2xl font-bold text-green-700">{prix}$</div>
              {tauxCdf > 0 && (
                <p className="text-xs text-gray-500">&#8776; {(prix * tauxCdf).toLocaleString()} CDF</p>
              )}
            </div>
          )}
        </div>
      ...
      <form onSubmit={handleSubmitForm} className="space-y-8">
        {/* ═══════ SECTION ASSUJETTI (identique carte-rose) ═══════ */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center space-x-3 mb-6">
        ...
        {/* ═══════ SECTION ENGIN (identique carte-rose) ═══════ */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center space-x-3 mb-6">
            <div className="bg-green-100 p-2 rounded-lg">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-600 text-sm">Détails du paiement et répartition</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
              ...
              <p className="text-3xl font-bold text-green-700">{prix}$</p>
              {tauxCdf > 0 && (
                <p className="text-xs text-green-600 mt-1">
                  &#8776; {(prix * tauxCdf).toLocaleString()} CDF
                </p>
              )}
            </div>

            {/* Mode paiement */}
            <div className="bg-gray-50 border-2 border-green-500 rounded-lg p-4 flex items-center">
              <DollarSign className="w-8 h-8 text-green-600 mr-3" />
              <div>
                <span className="font-bold text-green-700 text-lg">Espèces</span>
                <p className="text-xs text-green-600 mt-0.5">Paiement en espèces uniquement</p>
              </div>
            </div>

            {/* Numéro vignette physique (obligatoire) */}
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">
                N° Vignette physique <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
                value={numeroVignette}
                onChange={(e) => setNumeroVignette(e.target.value)}
                placeholder="Numéro physique de la vignette"
                required
                className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${vignetteError ? 'border-red-300' : 'border-gray-300'}`}
              />
              {vignetteError && (
                <p className="mt-1 text-xs text-red-500">{vignetteError}</p>
              )}
            </div>
            ...
            {repartition.length > 0 && (
              <div className="md:col-span-2">
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <span className="text-sm font-semibold text-gray-700">Répartition Bénéficiaires</span>
                </div>
                <div className="space-y-2">
                  {repartition.map((b, i) => (
                    <div
                    ...
                    >
                      <div>
                        <p className="text-sm font-medium text-gray-700">{b.nom}</p>
                        <p className="text-xs text-gray-500">
                          {b.type_part === "pourcentage" ? `${b.valeur_part}%` : `${b.valeur_part}$ fixe`}
                        </p>
                      </div>
                      <span className="text-sm font-bold text-green-600">{b.montant.toFixed(2)}$</span>
                    </div>
                  ))}
                  <div className="border-t border-gray-200 pt-2">
                    <div className="flex justify-between font-bold text-sm">
                      ...
                      <span className="text-green-600">
                        {repartition.reduce((sum, b) => sum + b.montant, 0).toFixed(2)}$
                      </span>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <span>Valider et Payer ({prix}$)</span>
          </button>
        </div>
      </form>
    </>
