...
export default function EnginInfo({ engin }: EnginInfoProps) {
  const infos = [
    ...
    },
    {
      ...
      label: "Énergie",
      value:
        Number(engin.energie) === 1
          ? "Essence"
          : Number(engin.energie) === 2
            ? "Diesel"
            : "Électrique",
    },
    {
    ...
    { icon: Cpu, label: "Numéro Moteur", value: engin.numero_moteur },
    {
      ...
      label: "Type Engin",
      value:
        Number(engin.type_engin) === 5
          ? "Moto"
          : Number(engin.type_engin) === 1
            ? "Voiture"
            : Number(engin.type_engin) === 2
              ? "Camion"
              : Number(engin.type_engin) === 3
                ? "Bus"
                : "Utilitaire",
    },
  ];
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="bg-emerald-50 rounded-lg px-4 py-2">
              ...
              <p className="text-sm font-bold text-emerald-700">
                {Number(engin.usage_engin) === 1
                  ? "Particulier"
                  : Number(engin.usage_engin) === 2
                    ? "Professionnel"
                    : "Transport"}
              </p>
            </div>
