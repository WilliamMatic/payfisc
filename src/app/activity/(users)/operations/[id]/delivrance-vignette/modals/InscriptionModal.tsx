...
import { inscrireAssujetti } from "@/services/vente-vignette/venteVignetteService";
...
export default function InscriptionModal({
  ...
  const marquesFiltrees = useMemo((): Marque[] => {
    ...
    if (engin.type_engin) {
      marques = marquesData.filter(
        (m: Marque) => m.type_engin === Number(engin.type_engin),
      );
    }
  ...
  const modelesFiltres = useMemo((): Modele[] => {
    ...
    if (engin.type_engin) {
      modeles = modeles.filter(
        (m: Modele) => m.type_engin === Number(engin.type_engin),
      );
    }

    if (engin.marque) {
      const marqueInfo: Marque | undefined = marquesData.find(
        (m: Marque) => m.nom === engin.marque,
      );
      if (marqueInfo) {
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      const result = await inscrireAssujetti({
        ...
        energie: String(engin.energie || "1"),
        usage_engin: String(engin.usage_engin || "1"),
        ...
        type_engin: String(engin.type_engin || "5"),
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start">
              ...
              <div className="text-sm text-amber-700">
                <p className="font-medium">
                  Paiement trouvé - Référence: {reference}
                </p>
                <p className="text-xs mt-1">
                  Veuillez enregistrer les informations du véhicule pour
