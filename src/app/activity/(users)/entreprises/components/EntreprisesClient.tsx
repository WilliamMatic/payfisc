...
export default function EntreprisesClient({
  ...
  const [selectedEntreprise, setSelectedEntreprise] =
    useState<EntrepriseType | null>(null);
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (result.status === "success") {
        ...
        setError(
          result.message || "Erreur lors de la recherche des entreprises"
        );
      }
    } catch (err) {
      setError("Erreur de connexion au serveur");
      ...
      <EntreprisesModals
        ...
        onAddEntreprise={async () => {
          if (
            ...
            setError(
              "Les champs raison sociale, NIF et registre de commerce sont obligatoires"
            );
            return;
          }
          ...
          try {
            ...
            const { addEntreprise } = await import(
              "@/services/entreprises/entrepriseService"
            );

            // Créer l'objet de données complet avec utilisateurId et siteCode
            ...
            const result = await addEntreprise(completeFormData);

            if (result.status === "success") {
              setSuccessMessage(
                result.message || "Entreprise ajoutée avec succès"
              );
              setFormData({
                raison_sociale: "",
                forme_juridique: "",
              ...
              setError(
                result.message || "Erreur lors de l'ajout de l'entreprise"
              );
            }
          } catch (err) {
            setError("Erreur de connexion au serveur");
        ...
        onEditEntreprise={async () => {
          ...
          setProcessing(true);
          try {
            const { updateEntreprise } = await import(
              "@/services/entreprises/entrepriseService"
            );
            const result = await updateEntreprise(
            ...
            if (result.status === "success") {
              setSuccessMessage(
                result.message || "Entreprise modifiée avec succès"
              );
              setShowEditModal(false);
              setSelectedEntreprise(null);
              setFormData({
        ...
        onDeleteEntreprise={async () => {
          ...
          setProcessing(true);
          try {
            const { deleteEntreprise } = await import(
              "@/services/entreprises/entrepriseService"
            );
            ...
            if (result.status === "success") {
              setSuccessMessage(
                result.message || "Entreprise supprimée avec succès"
              );
              setShowDeleteModal(false);
              setSelectedEntreprise(null);
        ...
        onToggleStatus={async () => {
          ...
          setProcessing(true);
          try {
            const { toggleEntrepriseStatus } = await import(
              "@/services/entreprises/entrepriseService"
            );
            const result = await toggleEntrepriseStatus(
              selectedEntreprise.id,
            ...
            if (result.status === "success") {
              setSuccessMessage(
                result.message || "Statut de l'entreprise modifié avec succès"
              );
              setShowStatusModal(false);
              setSelectedEntreprise(null);
