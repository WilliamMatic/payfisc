/**
 * Service pour la gestion de l'assurance moto
 * (Réplique de venteVignetteService adaptée pour l'assurance moto)
 */
...
/**
 * Enregistre un paiement d'assurance moto
 */
export const enregistrerPaiementAssurance = async (
  paiementData: {
    engin_id: number;
    particulier_id: number;
    montant: number;
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/assurance-moto/enregistrer_paiement.php`,
      {
        method: 'POST',
        credentials: 'include',
        headers: {
...
/**
 * Vérifie si une plaque a déjà une assurance valide
 */
export const verifierAssuranceExistante = async (
  plaque: string
): Promise<{
  status: 'success' | 'error';
  existe: boolean;
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/assurance-moto/verifier_assurance.php`,
      {
        method: 'POST',
        credentials: 'include',
        headers: {
    ...
    return {
      ...
      assurance_active: data.assurance_active || false,
      date_validite: data.date_validite,
      code_assurance: data.code_assurance,
      numero_assurance: data.numero_assurance,
      message: data.message,
    };
  } catch (error) {
    console.error('Vérification assurance error:', error);
    return {
      status: 'error',
      existe: false,
...
/**
 * Inscrit un nouvel assujetti et engin pour l'assurance moto
 */
export const inscrireAssujetti = async (
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/assurance-moto/inscrire_assujetti.php`,
      {
        method: 'POST',
        credentials: 'include',
        headers: {
...
/**
 * Vérifie une référence bancaire pour la délivrance groupée d'assurance
 */
export const verifierReferenceBancaire = async (
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/assurance-moto/verifier_reference.php`,
      {
        method: 'POST',
        credentials: 'include',
    ...
    console.error('Vérification référence bancaire assurance error:', error);
    return {
      status: 'error',
      message: 'Erreur réseau lors de la vérification',
    };
...
/**
 * Délivre une assurance dans le cadre d'une délivrance groupée (paiement bancaire)
 */
export const delivrerAssuranceGroupee = async (
  data: {
    reference_bancaire: string;
    numero_assurance: string;
    engin_id: number;
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/assurance-moto/delivrer_assurance_groupee.php`,
      {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    console.error('Délivrance assurance groupée error:', error);
    return {
      status: 'error',
...
/**
 * Récupère les assurances à renouveler
 */
export const getAssurancesARenouveler = async (
  filtres?: {
    seuil_jours?: number;
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/assurance-moto/assurances_a_renouveler.php`,
      {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    console.error('Assurances à renouveler error:', error);
    return {
      status: 'error',
...
/**
 * Renouvelle une assurance moto
 */
export const renouvelerAssurance = async (
  data: {
    assurance_id: number;
    utilisateur_id: number;
    utilisateur_name: string;
  ...
  data?: {
    ...
    duree_mois: number;
    montant: number;
    ancienne_assurance_id: number;
  };
}> => {
  try {
    const response = await fetch(
      `${API_BASE_URL}/assurance-moto/renouveler_assurance.php`,
      {
        method: 'POST',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
    ...
    console.error('Renouvellement assurance error:', error);
    return {
      status: 'error',
