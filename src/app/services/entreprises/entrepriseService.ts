...
const API_BASE_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:80/SOCOFIAPP/Impot/backend/calls/entreprises';
...
export const getEntreprises = async (): Promise<ApiResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL}/entreprises/lister_entreprises.php`, {
      method: 'GET',
      credentials: 'include',
      headers: {
 ...
 */
export const searchEntreprises = async (searchTerm: string): Promise<ApiResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL}/entreprises/rechercher_entreprises.php?search=${encodeURIComponent(searchTerm)}`, {
      method: 'GET',
      credentials: 'include',
      headers: {
        'Content-Type': 'application/json',
...
export const addEntreprise = async (entrepriseData: {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/entreprises/creer_entreprise.php`, {
      method: 'POST',
      credentials: 'include',
      body: formData,
    });
...
export const updateEntreprise = async (
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/entreprises/modifier_entreprise.php`, {
      method: 'POST',
      credentials: 'include',
      body: formData,
    });
...
export const deleteEntreprise = async (id: number): Promise<ApiResponse> => {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/entreprises/supprimer_entreprise.php`, {
      method: 'POST',
      credentials: 'include',
      body: formData,
    });
...
export const toggleEntrepriseStatus = async (
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/entreprises/changer_statut_entreprise.php`, {
      method: 'POST',
      credentials: 'include',
      body: formData,
    });
    ...
    if (!response.ok) {
      return {
        status: 'error',
        message: data.message || 'Échec du changement de statut de l\'entreprise',
      };
    }
