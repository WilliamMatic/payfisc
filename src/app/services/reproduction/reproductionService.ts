...
/**
 * Récupère l'historique des reproductions
 */
export const getHistoriqueReproductions = async (filters?: {
  date_debut?: string;
  date_fin?: string;
  site_id?: number;
  numero_plaque?: string;
  ...
  try {
    ...
    if (filters?.numero_plaque) params.append('numero_plaque', filters.numero_plaque);

    const response = await fetch(`${API_BASE_URL}/reproduction/historique_reproductions.php?${params}`, {
      method: 'GET',
      credentials: 'include',
      headers: {
    ...
    if (!response.ok) {
      return {
        status: 'error',
        message: data.message || 'Échec de la récupération de l\'historique',
      };
    }

    return data;
  } catch (error) {
    console.error('Get historique reproductions error:', error);
    return {
      status: 'error',
