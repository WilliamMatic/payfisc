<?php
/**
 * Endpoint: Annulation d'un paiement d'impôt
 */
...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        ...
        if (empty($input['reference_paiement'])) {
            echo json_encode([
                "status" => "error",
                "code" => "MISSING_PARAMETERS",
                "message" => "Paramètre 'reference_paiement' requis"
            ]);
            exit;
        }
        
        $result = $paymentAPI->annulerPaiementImpot($input['reference_paiement']);
    ...
    error_log("Erreur endpoint annulation paiement: " . $e->getMessage());
    echo json_encode([
        "status" => "error",
        "code" => "SYSTEM_ERROR", 
        "message" => "Erreur système: Impossible d'annuler le paiement."
    ]);
}
?>