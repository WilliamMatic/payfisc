<?php
/**
 * Endpoint pour l'export des statistiques
 */
...
try {
    ...
    if ($format === 'pdf') {
        $result = $statistiquesManager->exporterPDF($filtres);
    } elseif ($format === 'excel') {
        $result = $statistiquesManager->exporterExcel($filtres);
    } else {
        echo json_encode(["status" => "error", "message" => "Format d'export non supporté"]);
        exit;
    }
