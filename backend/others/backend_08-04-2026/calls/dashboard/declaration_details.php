...
try {
    ...
    $result = $dashboardManager->getDeclarationDetails($declarationId);
    echo json_encode($result);

} catch (Exception $e) {
    error_log("Erreur lors de la récupération des détails de déclaration : " . $e->getMessage());
    http_response_code(500);
    echo json_encode(["status" => "error", "message" => "Erreur système: Impossible de récupérer les détails de déclaration."]);
}
?>