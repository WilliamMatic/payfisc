...
HTML;
     ...
     *
     * @param int $id ID du particulier
     * @param array $data Données à modifier (nom_complet, telephone, adresse, nif, email)
     * @return array Tableau avec statut et message
     */
    public function modifierAssujettiSimple($id, $data)
    {
        $id = (int) $id;
        if ($id <= 0) {
            return ["status" => "error", "message" => "ID du particulier requis"];
        }
        ...
        try {
            ...
            if (!$this->particulierExisteParId($id)) {
                return ["status" => "error", "message" => "Particulier non trouvé"];
            }

            // Vérifier unicité téléphone si fourni
            if (!empty($telephone)) {
                $stmtTel = $this->pdo->prepare("SELECT id FROM particuliers WHERE telephone = :tel AND id != :id LIMIT 1");
                $stmtTel->execute([':tel' => $telephone, ':id' => $id]);
                if ($stmtTel->fetch()) {
                    return ["status" => "error", "message" => "Ce numéro de téléphone est déjà utilisé"];
                }
            }
            ...
            if (!empty($nif)) {
                $stmtNif = $this->pdo->prepare("SELECT id FROM particuliers WHERE nif = :nif AND id != :id LIMIT 1");
                $stmtNif->execute([':nif' => $nif, ':id' => $id]);
                if ($stmtNif->fetch()) {
                    return ["status" => "error", "message" => "Ce NIF est déjà utilisé"];
                }
            }
            ...
            if (!empty($email)) {
                $stmtEmail = $this->pdo->prepare("SELECT id FROM particuliers WHERE email = :email AND id != :id LIMIT 1");
            ...
            // Mise à jour
            $sql = "UPDATE particuliers SET nom = :nom, prenom = :prenom, telephone = :telephone, 
                    rue = :adresse, nif = :nif, email = :email, date_modification = CURRENT_TIMESTAMP 
                    WHERE id = :id";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute([
                ':nom' => $nom,
            ...
            $this->logAudit("Modification simplifiée du particulier ID $id: $nomComplet");
            ...
            error_log("Erreur modifierAssujettiSimple ID $id: " . $e->getMessage());
            ...
            return ["status" => "error", "message" => "Erreur système lors de la modification"];
        }
    }
