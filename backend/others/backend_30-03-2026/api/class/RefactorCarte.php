...
class RefactorCarte extends Connexion
{
    ...
    /**
     * Récupère les données complètes d'un paiement par son ID (DGRK)
     */
    public function recupererDonneesParIdDGRK($idDGRK, $siteNom)
    {
        try {
            $provinceId = null;
            $parent = null;
            
            // Récupérer le province_id du site via site_code
            $sqlSite = "SELECT s.province_id, s.id 
                        FROM sites s 
            ...
            $stmtSite->bindValue(':site_code', $siteNom, PDO::PARAM_STR);
            $stmtSite->execute();
            ...
            $sql = "SELECT 
                ...
                -- INNER JOIN utilisateurs u ON pm.utilisateur_id = u.id
                ...
                AND (pm.id = :id_dgrk OR e.numero_plaque = :numero_plaque)
                LIMIT 1";
            ...
            $stmt->execute([
                ...
                ':id_dgrk' => $idDGRK,
                ':numero_plaque' => $idDGRK
            ]);
            
            $donnees = $stmt->fetch(PDO::FETCH_ASSOC);

            if (!$donnees) {
                return ["status" => "error", "message" => "Identifiant DGRK non trouvé dans la base de données."];
            }
    ...
    /**
     * Met à jour les données d'un engin et particulier pour le refactor
     */
    public function mettreAJourDonneesRefactor($idDGRK, $donneesEngin, $donneesParticulier)
    {
        try {
            $this->beginTransactionSafe();

            // 1. Récupérer l'engin_id, particulier_id, site_id et l'ID du paiement
            $sqlPaiement = "SELECT 
                           pm.id as paiement_id,
                           pm.engin_id, 
                           pm.particulier_id, 
                           ...
                           FROM paiements_immatriculation pm
                           INNER JOIN engins e ON pm.engin_id = e.id 
                           WHERE pm.id = :id_dgrk OR e.numero_plaque = :id_dgrk
                           LIMIT 1";

            $stmtPaiement = $this->pdo->prepare($sqlPaiement);
            $stmtPaiement->execute([':id_dgrk' => $idDGRK]);
            
            $paiement = $stmtPaiement->fetch(PDO::FETCH_ASSOC);

            if (!$paiement) {
                throw new Exception("Paiement non trouvé avec l'ID DGRK ou numéro de plaque: " . $idDGRK);
            }

            $paiementId = $paiement['paiement_id']; // L'ID réel du paiement
            $enginId = $paiement['engin_id'];
            $particulierId = $paiement['particulier_id'];
            $siteId = $paiement['site_id'];
            $userId = $paiement['utilisateur_id'];

            // 2. Mettre à jour les données du particulier
            ...
            if (!$resultParticulier) {
                throw new Exception("Échec de la mise à jour des données du particulier");
            }
            ...
            // 4. Récupérer les données de l'engin actuel pour le numéro de plaque
            $sqlCurrentEngin = "SELECT numero_plaque FROM engins WHERE id = :engin_id";
            $stmtCurrentEngin = $this->pdo->prepare($sqlCurrentEngin);
            $stmtCurrentEngin->execute([':engin_id' => $enginId]);
            $currentEngin = $stmtCurrentEngin->fetch(PDO::FETCH_ASSOC);
            
            $numeroPlaque = $currentEngin ? $currentEngin['numero_plaque'] : '';

            // 5. Mettre à jour les données de l'engin
            ...
            if (!$resultEngin) {
                throw new Exception("Échec de la mise à jour des données de l'engin");
            }

            // 6. Vérifier si une entrée existe déjà dans carte_reprint pour ce paiement
            ...
            // 7. Mettre à jour ou insérer dans carte_reprint
            if ($existingReprint) {
            ...
            // 8. Log de l'action de refactor
            $this->logAuditRefactor($paiementId, $enginId, $particulierId);

            $this->commitSafe();

            // 9. Récupérer les données mises à jour pour le retour
            $sqlUpdated = "SELECT 
                          pm.id as paiement_id,
                          e.numero_plaque,
                          p.nom, 
            ...
            // 10. Ajouter également les données de carte_reprint
            if ($updatedData) {
    ...
    public function creerNouveauRefactor($numeroPlaque, $donneesEngin, $donneesParticulier, $siteCode)
    {
        try {
            ...
            if (!$siteData) {
                throw new Exception("Site non trouvé : $siteCode");
            }
            ...
            /* ================= PARTICULIER ================= */
            // Préparer les critères de recherche en ignorant les valeurs vides/tiret
            $searchParams = [];
            $searchConditions = [];

            if (!empty($donneesParticulier['telephone']) && $donneesParticulier['telephone'] !== '-') {
                $searchParams[':telephone'] = $donneesParticulier['telephone'];
                $searchConditions[] = 'telephone = :telephone';
            }

            if (!empty($donneesParticulier['email']) && $donneesParticulier['email'] !== '-') {
                $searchParams[':email'] = $donneesParticulier['email'];
                $searchConditions[] = 'email = :email';
            }

            if (!empty($donneesParticulier['nif']) && $donneesParticulier['nif'] !== '-') {
                $searchParams[':nif'] = $donneesParticulier['nif'];
                $searchConditions[] = 'nif = :nif';
            }

            // Vérifier si un particulier existe déjà avec l'un de ces identifiants
            ...
            if ($existingParticulier) {
                // UPDATE si le particulier existe
                $sqlParticulier = "UPDATE particuliers SET 
                                    nom = :nom,
                                    prenom = :prenom,
                                    telephone = :telephone,
                ...
                // INSERT si le particulier n'existe pas
                $sqlParticulier = "INSERT INTO particuliers
                            (nom, prenom, telephone, email, rue, nif, date_creation, date_modification)
                            VALUES (:nom, :prenom, :telephone, :email, :adresse, :nif, NOW(), NOW())";
            ...
            if (!$prefixe || !$numero) {
                throw new Exception("Format de plaque invalide : $numeroPlaque");
            }

            /* ================= SERIE ================= */
            $stmtSerie = $this->pdo->prepare(
                "SELECT id FROM series WHERE nom_serie = :serie LIMIT 1"
            );
            $stmtSerie->execute([':serie' => $prefixe]);
            $serieData = $stmtSerie->fetch(PDO::FETCH_ASSOC);

            if (!$serieData) {
                throw new Exception("Série inexistante : $prefixe");
            }
            ...
            if (!$serieItemData) {
                throw new Exception("Numéro de série invalide : $prefixe$numero");
            }

            $enginId = null;

            /* ================= ENGIN ================= */
            // D'abord vérifier si l'engin existe déjà
            $sqlCheck = "SELECT id FROM engins WHERE numero_plaque = :numero_plaque LIMIT 1";
            $stmtCheck = $this->pdo->prepare($sqlCheck);
            $stmtCheck->execute([':numero_plaque' => $numeroPlaque]);
            $existingEngin = $stmtCheck->fetch();

            if ($existingEngin) {
                // UPDATE si l'engin existe
                $sqlEngin = "UPDATE engins SET 
                                type_engin = :type_engin,
                                impot_id = :impot_id,
                                marque = :marque,
                ...
                // INSERT si l'engin n'existe pas
            ...
            $stmt = $this->pdo->prepare(
                "INSERT IGNORE INTO paiements_immatriculation
            ...
            // 5. Vérifier et créer/mettre à jour dans carte_reprint
            $sqlCheck = "SELECT id FROM carte_reprint WHERE numero_plaque = :numero_plaque LIMIT 1";
            $stmtCheck = $this->pdo->prepare($sqlCheck);
            $stmtCheck->execute([':numero_plaque' => $numeroPlaque]);
            ...
            // Retourner les données créées avec le site_nom
            return [
                "status" => "success",
     ...
     */
    private function logAuditRefactor($idDGRK, $enginId, $particulierId)
    {
        try {
