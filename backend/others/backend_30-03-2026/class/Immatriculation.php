...
class Immatriculation extends Connexion
{
    ...
    public function getNumeroPlaqueDisponibleSansReservation($utilisateurId)
    {
        try {
            ...
            $numeroPlaque = $serieItem['nom_serie'] . $serieItem['value'];
    ...
    public function traiterImmatriculation($data)
    {
        ...
        try {
            ...
            return ["status" => "error", "message" => "Erreur système (base de données)." . $e];
        } catch (Exception $e) {
            try {
                $this->rollbackSafe();
            } catch (Exception $ex) {
    ...
    public function rechercherPlaquesParTerme($searchTerm, $utilisateurId)
    {
        try {
            ...
            $sql = "SELECT 
                    ...
                    CONCAT(s.nom_serie, si.value) as numero_plaque,
                    si.statut,
                    s.nom_serie,
                    si.value
                    FROM serie_items si 
                    ...
                    AND (CONCAT(s.nom_serie, si.value) LIKE :search_term 
                         OR si.value LIKE :search_term)
                    ORDER BY 
                        CASE 
                            WHEN CONCAT(s.nom_serie, si.value) = :exact_term THEN 1
                            WHEN CONCAT(s.nom_serie, si.value) LIKE :start_term THEN 2
                            ELSE 3
                        END,
                    si.statut ASC,
                    si.id ASC
